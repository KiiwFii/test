...
public class AntiCheatCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender s, Command c, String a, String[] g) {
		...
		if(g.length == 0) {
			s.sendMessage(Color.Red + "Do '/rektanticheat help' for list of commands.");

			if (!(s instanceof Player)) {
				s.sendMessage(
						Color.Red + "This is for players only! Do /rektanticheat help to find a command you can do here.");
			}
			return true;
		} else {
		}
